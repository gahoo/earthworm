<template>
  <div class="flex w-full flex-col p-4">
          ...
          >
            ...
            >
              <template
                ...
                v-if="
                  isAuthenticated() &&
                  (coursePack.creatorId === userStore.user?.id ||
                    coursePack.uId === userStore.user?.id)
                "
              >
                <div
                  class="mt-4 flex justify-end space-x-2 border-t pt-2 dark:border-gray-700"
                  @click.stop
                >
                  <button
                    class="btn btn-outline btn-xs"
                    @click.stop="openEditModal(coursePack)"
                  >
                    Edit
                  </button>
                  <button
                    class="btn btn-outline btn-xs"
                    @click.stop="exportPack(coursePack.id, coursePack.title)"
                  >
                    Export
                  </button>
                  <button
                    class="btn btn-outline btn-error btn-xs"
                    @click.stop="removeCoursePack(coursePack.id)"
                  >
                    Delete
                  </button>
                </div>
    ...
    >
      <UCard>
        <h3 class="mb-4 text-lg font-bold">
          {{ editingPackId ? "Edit Course Pack" : "Create Course Pack" }}
        </h3>
        <div class="space-y-4">
          <div>
    ...
    >
      <UCard>
        <h3 class="mb-4 text-lg font-bold text-gray-800 dark:text-white">Generate Course via AI</h3>
        ...
        <div class="space-y-4">
          <div class="form-control">
            ...
            <div class="max-h-48 overflow-y-auto rounded border border-base-300 bg-base-200 p-2">
              <label
                v-for="mat in availableMaterials"
                :key="mat.name"
                class="label flex cursor-pointer justify-start space-x-3"
              >
                <input
                  type="checkbox"
                  :value="mat.name"
          ...
          <div class="form-control">
            <label class="label"
              ><span class="label-text">Additional Prompt (Optional)</span></label
            >
            <textarea
              v-model="aiPrompt"
              placeholder="E.g., Focus on business English vocabulary..."
          ...
          <div class="form-control">
            <label class="label"
              ><span class="label-text">API Key (Optional if configured on backend)</span></label
            >
            <input
          ...
          <div class="form-control">
            ...
            <input
              ...
              type="text"
              :placeholder="aiSettings.provider === 'openai' ? 'gpt-4o' : 'gemini-1.5-pro'"
              class="input input-bordered w-full"
            />
          </div>
        </div>
        ...
        <div class="modal-action mt-6">
          ...
          <button
            ...
            @click="generateViaAi"
            :disabled="generating || (selectedMaterials.length === 0 && !aiPrompt.trim())"
          >
            <span
              v-if="generating"
...
const saveNewPack = async () => {
  try {
    ...
    toast.error(
      editingPackId.value ? "Failed to update course pack" : "Failed to create course pack",
    );
  }
};

const removeCoursePack = async (id: string) => {
  if (!confirm("Are you sure you want to delete this course pack? All its courses will be lost."))
    return;
  try {
  ...
  try {
    ...
    if (data.courses && data.courses.length > 0) {
      ...
      data.courses.forEach((course: any, index: number) => {
        coursesFolder?.file(
          `${index + 1}_${course.title.replace(/\s+/g, "_")}.json`,
...
const generateViaAi = async () => {
  ...
  const toastId = toast.loading(
    "AI is analyzing materials and generating course content... This may take a minute.",
  );
  ...
  try {
    ...
    toast.error(
      "AI generation failed. Please check your API keys, network connection, or try smaller files.",
